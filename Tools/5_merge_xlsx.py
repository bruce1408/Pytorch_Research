...
# output_file = "./od_bev_0220_quant_result_analysis.xlsx"
...
#     # 找出除了 'label' 外在两个表中重复的列
#     duplicate_cols = set(df_base.columns) & set(df_current.columns)
#     duplicate_cols.discard('label')
    
#     # 如果存在重复列，对 df_current 中的重复列进行重命名
#     if duplicate_cols:
#         # 使用文件名（去掉扩展名）作为后缀
#         file_suffix = os.path.splitext(os.path.basename(file))[0]
#         rename_dict = {col: f"{col}_{file_suffix}" for col in duplicate_cols}
...
# # 设置要读取的Excel文件所在的目录和输出文件路径
# # excel_dir = "/Users/brucetrie/Downloads/"
# excel_dir = "/Users/brucetrie/Downloads/8620项目/370_parking_obstacle/25_0219_model/excel_file_merge"
